export * from './engine-v031.js';
import {
  calculateTeamPower,
  createBattle as createHexBattle,
  seededRandom,
  teamCapForRound
} from './engine-v031.js';
...
export function readDifficultyState() {
  const state = storageGet(DIFFICULTY_STORAGE_KEY, {});
  return state && typeof state === 'object' ? state : {};
}

function writeDifficultyState(state) {
  storageSet(DIFFICULTY_STORAGE_KEY, state);
}
      ...
      for (let a = 0; a < archetypes.length; a += 1) {
        for (let offset = 0; offset < Math.min(3, archetypes[a].length); offset += 1) {
          ...
          enumerateStars(count, maxStar, (stars) => {
            const team = ids.map((unitId, index) => ({
              uid: `enemy-${round}-${a}-${offset}-${index}`,
              unitId,
              star: stars[index],
              slot: index
            }));
...
function currentWizardLevel() {
  const save = storageGet(SAVE_KEY, null);
...
export function generateEnemyTeam({ round = 1, playerPower = 0 } = {}) {
  ...
  // A defeat is a puzzle retry: never secretly replace the opponent.
  if (state.lastWinner === 'enemy' && Array.isArray(state.lastEnemyTeam) && state.lastEnemyTeam.length) {
    return clone(state.lastEnemyTeam);
  }

  const cached = activeChallenge(state, round);
  if (cached) return clone(cached.team);

  const previousPower = state.lastWinner === 'player' && state.lastPlayerPower
    ? state.lastPlayerPower
    : Math.max(230, Number(playerPower) || 230);
  ...
  state.challenges ||= {};
  state.challenges[String(round)] = {
    round,
    basisPower: previousPower,
    coefficient,
    targetPower,
    actualPower: built.power,
    team: clone(built.team)
  };
  writeDifficultyState(state);
  return clone(built.team);
}

function currentRoundFromSave() {
  const save = storageGet(SAVE_KEY, null);
...
export function createBattle(options) {
  ...
  const round = currentRoundFromSave();
  ...
  battle.step = (dt) => {
    ...
    if (battle.finished && !recorded) {
      recorded = true;
      const state = readDifficultyState();
      state.lastWinner = battle.winner;
      state.lastPlayerPower = playerPower;
      state.lastEnemyTeam = enemySnapshot;
      state.lastRound = round;
      state.lastBattleAt = Date.now();
      writeDifficultyState(state);
      if (typeof window !== 'undefined') {
        window.__mergeLastBattleResult = battle.winner;
