import './v033-state.js?v=0.4.0';
import './v034-ui.js?v=0.4.0';
import { enhanceFormation } from './v032-formation.js?v=0.4.0';
import { enhanceCombatUi, refreshCombatBadges } from './v032-combat-ui.js?v=0.4.0';
...
const observer=new MutationObserver((mutations)=>{
  ...
  for(const mutation of mutations){
    if(mutation.type==='attributes'&&mutation.target.matches?.('.hpbar i')){
      ...
      const meaningful=[...mutation.addedNodes].some((node)=>node.nodeType===1&&!node.classList?.contains('tactical-deploy-layer')&&!node.classList?.contains('enemy-deploy-layer')&&!node.classList?.contains('upgrade-preview')&&!node.classList?.contains('daily-rotation-note')&&!node.classList?.contains('battle-report-v04'));
