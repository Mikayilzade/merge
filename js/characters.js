...
export function unitArt(unitId, { className = '', label = '', decorative = false } = {}) {
  ...
  if (!def?.art) return `<span class="unit-art-fallback" aria-label="Персонаж">♟</span>`;
  const aria = decorative ? 'aria-hidden="true"' : `role="img" aria-label="${label || def.name}"`;
  const svg = `<svg class="unit-art unit-art-${unitId} ${className}" viewBox="0 0 120 140" ${aria} xmlns="http://www.w3.org/2000/svg">
  ...
  return svg.includes('undefined') ? `<span class="unit-art-fallback" aria-label="${def.name}">♟</span>` : svg;
}

export function wizardArt({ className = '' } = {}) {
  return `<svg class="wizard-art ${className}" viewBox="0 0 180 210" role="img" aria-label="Архонт" xmlns="http://www.w3.org/2000/svg">
