import { hexKey, hexToPercent } from './hex-grid.js';
...
export function readStoredFormation(storage) {
  ...
  const parsed = readJsonStorage(FORMATION_STORAGE_KEY, {});
  return parsed && typeof parsed === 'object' ? parsed : {};
}

export function playerDeploymentCell(slot, formation = {}) {
  const raw = Number(formation[String(slot)]);
  if (Number.isInteger(raw) && raw >= 0 && raw < PLAYER_DEPLOY_CELLS.length) return { ...PLAYER_DEPLOY_CELLS[raw] };
...
export function applyStoredFormation(battle, formation = readStoredFormation()) {
  const used = new Set();
  let playerIndex = 0;
  let enemyIndex = 0;
  for (const unit of battle.units) {
    ...
    unit.facingIndex = unit.side === 'player' ? 2 : 5;
    used.add(hexKey(unit.col, unit.row));
  }
  return battle;
}

export function exposeCombatState(battle) {
...
export function wrapReadableBattleSpeed(battle) {
