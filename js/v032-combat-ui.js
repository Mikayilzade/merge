...
function refreshCopy(root=document){const eyebrow=root.querySelector('.hero-v2 .eyebrow');if(eyebrow)eyebrow.textContent='ПРОТОТИП 0.4 · МОДУЛЬНЫЙ ДВИЖОК И ТАКТИЧЕСКИЙ ИИ';root.querySelectorAll('.guide-steps article p').forEach((p)=>{if(p.textContent.includes('Верхний ряд ближе к противнику')||p.textContent.includes('Расставь бойцов на своей стороне'))p.textContent='Нижние три ряда — твои 21 стартовый гекс. Выбери героя, затем любой свободный гекс. Враг заранее виден сверху и не меняется от твоей перестановки.';});refreshHelp(root);}
